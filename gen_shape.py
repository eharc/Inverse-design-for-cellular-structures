...
if __name__ == "__main__":
    ...
    DATA1=np.loadtxt('D:\STAGE2A\MMAE\PHYSICINFORMED\data00255.txt')
    X=DATA1[:,:1]   
    C=DATA1[:,1:]
    ...
    # Train GAN
    model = Model(noise_dim, X.shape[1], C.shape[1],C_test.shape[1],X_test, n_classes=2)
    if args.mode == 'train':
        timer = ElapsedTimer()  
        model.train(X_train, C_train, X_test, C_test,X,C, batch_size=batch_size, train_steps=train_steps, 
    ...
    opt_dir = 'D:\STAGE2A\MMAE\PHYSICINFORMED'
    C_tgt = C_test
