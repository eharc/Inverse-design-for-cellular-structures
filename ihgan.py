...
class Model(object):
    ...
    def __init__(self, dim_Z, dim_X, dim_C, n_classes=2):
    ...
    def generator(self, z, c, reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('Generator', reuse=reuse):
            ...
            # x1 and x2 are the two geometrical parameters
            x1 = tf.layers.dense(x, self.n_classes-1)
            x2 = tf.layers.dense(x, self.n_classes-1)
                
            x = tf.concat([x1, x2], axis=1, name='gen')
    ...
    def train(self, X_train, C_train, X_test, C_test, train_steps=5000, batch_size=32, 
        ...
        self.normalizer_X = Normalizer(data=X_train)
...
#        g_loss=tf.reduce_mean(tf.abs(self.x_fake[:,0]-self.x[:,0]))
        # L1 loss for c
        ...
        #empty array for the generator output
        ...
        #file with every generation from the generator at each trainstep
